...
export default function AllowancesScreen({ route, navigation }) {
  ...
  const {
    ...
    defaultValues: {
      km: createHourReg?.km,
      kmExtraTypes: createHourReg?.kmExtraTypes || [],
      kmDescription: createHourReg?.kmDescription,
      toll: createHourReg?.toll,
      parking: createHourReg?.parking,
      dietType: createHourReg?.dietType,
      dietFrom: createHourReg?.dietFrom,
      dietTo: createHourReg?.dietTo,
      outlay: createHourReg?.outlay,
      outlayDescription: createHourReg?.outlayDescription,
      outlayType: createHourReg?.outlayType,
      travelTime: createHourReg?.travelTime,
    },
  });

  const handleCreate = async (data) => {
    try {
      data = { ...createHourReg, ...data };
      const res = await apis.createHourReg(data);
      store.dispatch({ type: SET_CREATE_HOUR_REG, payload: {} });
