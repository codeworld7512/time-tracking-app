...
export default function MovableHolidayEditScreen({ navigation, route }) {
  ...
  const init = async () => {
    try {
      const { data } = await apis.getMovableHolidayHour(id);
  ...
  const handleUpdate = async (data) => {
    try {
      await apis.updateMovableHolidayHour(id, data);
      setOpenDialog({ ...openDialog, delete: false });
      showSnackbar({
        type: "open",
        message: i18n.t("movableholidayHaveUpdatedSuccessfully"),
